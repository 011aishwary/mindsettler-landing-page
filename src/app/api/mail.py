...
def main(email, name, date, timee, mode, status , key , secret ):
...
def handler(request):
    try:
        ...
        key = os.environ.get("MINDSETTLER_EMAIL")
        secret = os.environ.get("MINDSETTLER_PASS")

        if not key or not secret:
            return {
                "statusCode": 500,
                "body": json.dumps({ "error": "Missing env vars" })
            }
        ...
        timee = body.get("time", "").strip()
        mode = body.get("mode", "").strip()
        status = body.get("status", "").strip()
        print(email)
        print(f"Preparing email for {name} <{email}> on {date} at {timee} ({mode}) and status: {status}")

        main(email, name, date, timee, mode, status , key , secret )
